...
with open("results_morpholex.csv") as f:
    for r in csv.DictReader(f):
        r["comp"] = float(r["comp"]); r["zipf"] = float(r["zipf"])
        r["freq"] = float(r["freq"]); r["root_free"] = r["root_free"] == "True"
        rows.append(r)
...
ax.hist([r["comp"] for r in pref], bins=bins, density=True, alpha=.55,
        color=PREFIX_C, label=f"prefix (n={len(pref)})")
ax.hist([r["comp"] for r in suff], bins=bins, density=True, alpha=.55,
        color=SUFFIX_C, label=f"suffix (n={len(suff)})")
...
ax.legend()
fig.tight_layout(); fig.savefig("fig1_distributions.png", dpi=130)
...
ax.set_xlabel("word frequency (Zipf band)"); ax.set_ylabel(f"novelty rate (comp < {T})")
ax.set_title("The prefix advantage grows with frequency (common prefix words lexicalize)")
ax.legend()
fig.tight_layout(); fig.savefig("fig2_frequency.png", dpi=130)

# --- Figure 3: type / token / bound-root summary ---------------------------
cats = ["type", "token\n(usage-weighted)", "bound-root\nonly"]
pv = [stats["type_novelty"]["prefix"], stats["token_novelty"]["prefix"],
      stats["boundroot_novelty"]["prefix"]]
sv = [stats["type_novelty"]["suffix"], stats["token_novelty"]["suffix"],
...
for b in list(b1)+list(b2):
    ax.text(b.get_x()+b.get_width()/2, b.get_height()+.005, f"{b.get_height():.0%}",
...
ax.legend()
fig.tight_layout(); fig.savefig("fig3_summary.png", dpi=130)
