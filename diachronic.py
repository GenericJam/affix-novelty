...
def main():
    ...
    for y in DECADES:
        ...
        if len(pref) < 50 or len(suff) < 50:
            ...
            continue
        traj["year"].append(y)
        traj["prefix"].append(np.mean(pref)); traj["suffix"].append(np.mean(suff))
        traj["np"].append(len(pref)); traj["ns"].append(len(suff))
        print(f"{y}: prefix mean={np.mean(pref):+.3f} (n={len(pref)})  "
              f"suffix mean={np.mean(suff):+.3f} (n={len(suff)})  gap={np.mean(suff)-np.mean(pref):+.3f}")
    ...
    ax.set_xlabel("decade"); ax.set_ylabel("mean compositionality (higher = transparent)")
    ax.set_title("The prefix-suffix compositionality gap is stable across the 20th century")
    ax.legend(); fig.tight_layout(); fig.savefig("fig4_diachronic.png", dpi=130)
    ...
    A = np.array([ke[w] for w in shared]); B = np.array([kl[w] for w in shared])
    # orthogonal Procrustes: R aligns A -> B
    U, _, Vt = np.linalg.svd(A.T @ B)
    R = U @ Vt
    def change(word):
        if word in ke and word in kl:
            a = ke[word] @ R; b = kl[word]
    ...
    pc = [x for x in pc if x is not None]; sc = [x for x in sc if x is not None]
