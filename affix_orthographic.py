...
CANDS = [
    w for w in common if zipf_frequency(w, "en") >= WORD_FREQ_FLOOR and len(w) >= 4 and w in KV
]
...
print(
    f"  {sum(r['side'] == 'prefix' for r in records)} prefix decompositions, "
...
def summarize(side):
    ...
    out = {"side": side, "n": len(rs), "mean": comps.mean(), "median": np.median(comps)}
    for t in THRESHOLDS:
...
print(f"  {'zipf':12s}{'pref n':>8s}{'pref novel':>12s}{'suf n':>8s}{'suf novel':>12s}")
...
def affix_table(side, top=12):
    ...
    for af, n, m, rate in [o for o in out if o[1] >= 15][:top]:
        exs = sorted(
            [r for r in scored if r["side"] == side and r["affix"] == af], key=lambda r: r["comp"]
        )[:4]
