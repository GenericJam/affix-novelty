...
def build(lang, kv, prefixes, suffixes, n_words=50000,
          word_floor=2.0, root_floor=2.8, min_root=4, min_affix=2):
    ...
    cands = [w for w in common
             if zipf_frequency(w, lang) >= word_floor and len(w) >= 5 and w in kv]
    ...
    for w in cands:
        for af in pref:                       # longest affix first
            if w.startswith(af) and len(w) - len(af) >= min_root:
                r = root_of(w[len(af):])
                if r and r != w:
                    entries.append(dict(word=w, side="prefix", affix=af, root=r,
