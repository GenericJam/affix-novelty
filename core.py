...
def learn_offsets(entries, kv, min_pairs=8):
    ...
    for key, grp in by.items():
        ...
        med = np.median(comps)
        keep = [d for d, c in zip(diffs, comps, strict=False) if c >= med]
...
def summarize(scored, bands=((1.5, 2.5), (2.5, 3.0), (3.0, 3.5), (3.5, 4.0), (4.0, 7.0))):
    ...
    for t in THRESHOLDS:
        out[f"type_<{t}"] = {"prefix": _novelty(pref, t, False), "suffix": _novelty(suff, t, False)}
        out[f"token_<{t}"] = {"prefix": _novelty(pref, t, True), "suffix": _novelty(suff, t, True)}
    out["mean_comp"] = {
        "prefix": float(np.mean([e["comp"] for e in pref])),
    ...
    if pref and suff:
        _, p = mannwhitneyu(
            [e["comp"] for e in pref], [e["comp"] for e in suff], alternative="two-sided"
        )
        ...
        out["by_freq_<0.15"].append(
            {
                ...
                "n_prefix": len(pr),
                "n_suffix": len(su),
                "prefix": _novelty(pr, 0.15, False),
                "suffix": _novelty(su, 0.15, False),
            }
        )
    return out
...
def print_summary(name, s):
    ...
    print(
        f"  mean comp: prefix={s['mean_comp']['prefix']:+.3f}  "
    ...
    for r in s["by_freq_<0.15"]:
        print(
            f"    {r['band']:9s} prefix={r['prefix']:.1%} (n={r['n_prefix']:4d})"
