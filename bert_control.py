...
def asymmetry(scored):
    ...
    return {
        ...
        "suffix_mean": s.mean(),
        "gap": s.mean() - p.mean(),
        "prefix_nov": float((p < 0.15).mean()),
        "suffix_nov": float((s < 0.15).mean()),
        "p": pv,
    }
...
def main():
    ...
    print(
        f"{'model':20s}{'pfx_mean':>9s}{'sfx_mean':>9s}{'gap':>8s}"
    ...
    for m in MODELS:
        ...
        rows[m] = a
        print(
            f"{m:20s}{a['prefix_mean']:+9.3f}{a['suffix_mean']:+9.3f}{a['gap']:+8.3f}"
            f"{a['prefix_nov']:9.1%}{a['suffix_nov']:9.1%}{ratio:7.2f}{a['p']:10.1e}"
        )
    import json

    with open("stats_bert_control.json", "w") as f:
        json.dump({m: {k: float(v) for k, v in a.items()} for m, a in rows.items()}, f, indent=2)
