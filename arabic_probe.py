...
def english():
    ...
    coh = {"prefix": [], "suffix": []}
    for _root, members in fam.items():
        if len(members) < 3:
            continue
    ...
    for side in ("prefix", "suffix"):
        ...
        print(
            f"  {side}-only families (n={len(arr):4d}): mean within-family "
...
def arabic():
    ...
    print(f"  random-pair baseline coherence: {base:+.3f}")
    print(
        f"  mean within-root coherence: {np.mean(coh):+.3f}  "
